...
def read_tree_BITSC2(basename):
    tree={}
    SNV_to_region = map_SNV_regions("out_COMPASS/"+basename+"_variants.csv")
    with open("out_BITSC2/"+basename+"_treeBITSC2.csv","r") as file:
        parents=[]
        for line in file:
    ...
    with open("out_BITSC2/"+basename+"_originsBITSC2.csv","r") as file:
        for line in file:
            linesplit = line.split(" ")
...
def is_SNVsupported(node,tree,nodes_genotypes,setNodes):
    # A node is SNV-supported if it (or one of its descendants) contains a SNV, a CNLOH or a CNV resulting in a LOH.
    global SNV_to_region
    if len(tree["muts"][node])>0:
        setNodes.add(node)
    for CNV in tree["CNVs"][node]:
        region,sign = CNV
        for locus in range(tree["n_muts"]):
            if SNV_to_region[locus]==region:
                if sign == -1 and locus in nodes_genotypes[node]: # CNV results in LOH
                    setNodes.add(node)
    for CNLOH in tree["CNLOHs"][node]:
        for locus in range(tree["n_muts"]):
            if SNV_to_region[locus]==CNLOH:
                if locus in nodes_genotypes[node]:
                    setNodes.add(node)
...
def compute_error_rates(basename,nseeds,output):
    ...
    for s in range(nseeds):
        tree_true = read_tree_gv("out_COMPASS/"+basename+"_"+str(s)+"_treeTRUE.gv")
        tree_COMPASS = read_tree_gv("out_COMPASS/"+basename+"_"+str(s)+"_treeCOMPASS.gv")
        tree_BITSC2 = read_tree_BITSC2(basename+"_"+str(s))
        global SNV_to_region
        SNV_to_region = map_SNV_regions("out_COMPASS/"+basename+"_"+str(s)+"_variants.csv")
