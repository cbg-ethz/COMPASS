...
def generate_data(basename,n_nodes,n_cells,n_muts,n_CNV,nodeprobs_concentration,dropout_rate_avg,dropout_rate_sigma,\
    ...
    # Assign mutation to nodes. Each node must have at least one mutation
    if (n_muts<n_nodes-1):
        raise Exception("Must have more muts than nodes.")
    shuffled_muts=list(range(n_muts))
    np.random.shuffle(shuffled_muts)
    for i in range(0,n_nodes-1): # exclude root here ?
        muts[i+1].append(shuffled_muts[i])
    for i in range(n_nodes-1,n_muts):
    ...
    loci_with_CNV = sorted(np.random.choice(n_muts,n_CNV,replace=False))
    # Assign CNVs to nodes
    for i in range(n_CNV):
        locus = loci_with_CNV[i]
        if np.random.choice([True,False]):
            # Copy Number Loss: affects the ref allele, and can be placed anywhere in the tree.
            node = np.random.choice(range(1,n_nodes))
            CNVs[node].append( ( locus , -1 , 0 ) )
        else:
            # Copy Number Gain
            allele = np.random.choice([0,1])
            if allele==0: # affects the ref allele: can be placed anywhere in the tree
                node = np.random.choice(range(1,n_nodes))
            else: # affects the alt allele: must be in the same node as the mutation
                for n in range(n_nodes):
                    if locus in muts[n]:
                        node=n
            CNVs[node].append( ( locus , +1 , allele ) )
    ...
    region_probabilities = np.abs(np.random.normal(1,regionprob_sigma,n_muts))
    ...
    d["REGION"] = ["Region"+str(x) for x in range(n_muts)]
    d["NAME"] = ["Mut"+str(x) for x in range(n_muts)]
    for j in range(n_cells):
        d[j]=[]
    for i in range(n_muts):
        for j in range(n_cells):
    ...
    df_regions = pd.DataFrame(depth,index = [str(i)+"_Region"+str(i) for i in range(n_muts)]).astype(int)
...
if __name__=="__main__":
    ...
    parser.add_argument('--nCNV', type = int,default = 2, help='Number of CNVs')
    ...
    generate_data(args.o,n_nodes=args.nnodes,n_cells=args.ncells,n_muts=args.nmuts,n_CNV=args.nCNV, nodeprobs_concentration=args.nodeprobconcentration,\
