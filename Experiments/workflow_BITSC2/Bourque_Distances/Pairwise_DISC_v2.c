 ...
 *   This porgram computes the pairwise CASet measures between a 
     set of labeled trees with the same root, which is the first
     root in the imput trees, and their pairwise distances are output
...
*  
* 
* Complie command: gcc -lm Pairwise_CASet_v2.c -o PW_CASet
* Run command: ./PW_CASet  <inputtrees_file> <dist_output_file> <no_muts>
...
void Comput_Dist_Ancestors(short root,  short no_nodes, short no_muts,
   ...
   for (i=0; i<no_muts; i++) {
      ...
      if (node1!=-1 && node1!=node2) {
        ...
        for (k=0; k<no_nodes; k++) {
          if (dist[root][k]+dist[k][node1]==dist[root][node1] &&
               (k==node2 || dist[root][k]+dist[k][node2]!=dist[root][node2])) {
                /* printf("k=%d\n", k); */
            if (k!=node1)
            for (m=0; m<no_muts; m++) {
              if (node_muts[k][m]=='1') dist_ancestors[i][j][m]=node_muts[k][m];
...
short DISC(float *t_dist, short no_muts, short no_muts_ij,  
  short map1[], short map2[],
 char *dist_ans1[][MAXSIZE], char *dist_ans2[][MAXSIZE]) {
   ...
   for (i=0; i<no_muts; i++) {
     for (j=0; j<no_muts; j++) {
       if (i!=j && (map1[i]!=-1 ||  map2[i]!=-1) 
             && (map1[j]!=-1 ||  map2[j]!=-1)  ) {
         diff=0;  cup=0;
        /*  printf("%s\n%s\n", dist_ans1[i][j], dist_ans2[i][j]); */
         for (m=0; m<no_muts; m++) {
   ...
   if (no_muts_ij!=0 && no_muts_ij!=1) { 
        score=score/no_muts_ij; score=score/(no_muts_ij-1);
   }
   *t_dist=score;
   return 0;
}
...
/*
 * Compile command: gcc -lm Pairwise_DISC_v2.c -o PW_DISC
 * Run command:     ./PW_DISC <input_file> <output_file> <no_muts>
...
short no_muts_ij;
 ...
 Comput_Mutation(graphs[i][0][0]+1, node_labels[i], total_no_muts, node_muts[i],
  ...
  Comput_Dist_Ancestors(root, graphs[i][0][0]+1, total_no_muts, distance[i], 
   node_muts[i], mut_to_node_map[i],  dist_ancestors[i]); 
  ...
  for (j=0; j<total_no_muts; j++) {
     for (k=0; k<total_no_muts; k++) {
         printf("mut(%d, %d): %s\n", j, k, dist_ancestors[i][j][k]);
     }
  }
  */
 }
   ...
   for (i=0; i<count; i++) {
      for (j=i+1; j<count; j++) {     
       ...
       no_muts_ij=Comput_mut_in(total_no_muts,  
        ...
        t_dist=0.0;
     /*
     printf("Tree %d vs %d:  no_mut_ij %d\n", i, j, no_muts_ij);
     */
    interval=DISC(&t_dist, total_no_muts, no_muts_ij,
       mut_to_node_map[i],  mut_to_node_map[j],  dist_ancestors[i], dist_ancestors[j]); 
       fprintf(t_OUT, "%f\n", t_dist);
       /*
       fprintf(t_OUT, "%f\n", t_dist);
       */
