...
*   Complie command: gcc -lm Pairwise_AD_v2.c -o PW_AD
*  The run command: ./PW_AD  <inputtrees_file> <dist_output_file> no_nodes
*  
...
void Comput_Ancestors(short root,  short no_nodes, short no_muts,
 ...
 *        */
   ...
   for (i=0; i<no_muts; i++) {
      ...
      ancestors[i]=(char *)malloc(no_muts+1);
      strcpy(ancestors[i], non_muts);
      if (node1!=-1) {
        for (k=0; k<no_nodes; k++) {
          if (k!=node1 && dist[root][k]+dist[k][node1]==dist[root][node1]) {
                /* printf("k=%d\n", k); */
            for (m=0; m<no_muts; m++) {
              if (node_muts[k][m]=='1') ancestors[i][m]=node_muts[k][m];
            }
         }
       } /* for */
     } /* if node1 */
...
/*
void  Comput_Dist_Ancestors(root, graphs[i][0][0]+1, MAXSIZE-1, distance[i], 
   node_muts[i], mut_to_node_map[i],  dist_ancestors[i]) {
*/
void Comput_Dist_Ancestors(short root,  short no_nodes, short no_muts,
   short dist[][MAXSIZE], char node_muts[][MAXSIZE], short mut_to_node_map[], 
   char *dist_ancestors[][MAXSIZE]) {
   short i, j, k, m;
   char non_muts[MAXSIZE];
   short node1, node2;
   /*
   ...
   for (i=0; i<no_muts; i++) {
      ...
      for (j=0; j<no_muts; j++) {
        ...
        dist_ancestors[i][j]=(char *)malloc(no_muts+1);
        strcpy(dist_ancestors[i][j], non_muts);
        ...
        if (node2==-1) {
         for (k=0; k<no_nodes; k++) {
           if (dist[root][k]+dist[k][node1]==dist[root][node1]) {
            if (k!=node1 && k!=node2)
            for (m=0; m<no_muts; m++) {
              if (node_muts[k][m]=='1') dist_ancestors[i][j][m]=node_muts[k][m];
            }
           }
         }
        } else {
...
short DISC(float *t_dist, short no_muts, char *dist_ans1[][MAXSIZE],
  char *dist_ans2[][MAXSIZE]) {
   ...
   for (i=0; i<no_muts; i++) {
     for (j=0; j<no_muts; j++) {
       if (i!=j) {
         ...
         /* printf("%s\n%s\n", c_ances1[i][j], c_ances2[i][j]); */
         for (m=0; m<no_muts; m++) {
         if (dist_ans1[i][j][m]=='1'&& dist_ans2[i][j][m]=='a') {diff++; cup++;} 
         if (dist_ans1[i][j][m]=='a' && dist_ans2[i][j][m]=='1') {diff++; cup++;} 
          if (dist_ans1[i][j][m]=='1' && dist_ans2[i][j][m]=='1') {  cup++; } 
         }
         if (cup!=0) score=score+diff/(float)cup; 
         /* printf("%d -- %d\n", diff, cup); */
       } /* if */
   ...
   if (no_muts!=0 && no_muts!=1) { 
        score=score/no_muts; score=score/(no_muts-1);
   }
   *t_dist=score;
   return 0;
}

short AD(float *t_dist, short no_muts,  short no_muts_ij, short map1[],
   short map2[],   char *ans1[], char *ans2[]) {
   ...
   for (i=0; i<no_muts; i++) {
        ...
        if (map1[i]!=-1 ||  map2[i]!=-1){
          ...
          */
         for (m=0; m<no_muts; m++) {
         if (ans1[i][m]=='1'&& ans2[i][m]=='a') {diff++; cup++; } 
         if (ans1[i][m]=='a' && ans2[i][m]=='1') {diff++; cup++;} 
         if (ans1[i][m]=='1' && ans2[i][m]=='1') {  cup++; } 
         }
         if (cup!=0) score=score+diff/(float)cup; 
          /*
          printf("                   %d -- %d\n", diff, cup); 
   ...
   if (no_muts_ij!=0) score=score/no_muts_ij;
   *t_dist=score;
   return 0;
}
...
char  *dist_ancestors[MAX_NO_TREES][MAXSIZE][MAXSIZE];
char  *ancestors[MAX_NO_TREES][MAXSIZE];
short total_no_muts;
short no_muts_ij;
 ...
 */
  ...
  Comput_Mutation(graphs[i][0][0]+1, node_labels[i], total_no_muts, node_muts[i],
  mut_to_node_map[i]); 
  ...
  */
  ...
  Comput_Ancestors(root, graphs[i][0][0]+1, total_no_muts, distance[i], 
   node_muts[i], mut_to_node_map[i],  ancestors[i]); 


  /*
  printf("---------------hhhh----\n");
  for (j=0; j<total_no_muts; j++) {
         printf("mut(%d, ): %s\n", j, ancestors[i][j]);
  }
  */
 }
   ...
   for (i=0; i<count; i++) {
      for (j=i+1; j<count; j++) {     
        ...
        */
        ...
        /*
        printf("trees %d %d, no_mut_ij %d\n", i, j,  no_muts_ij);
         */
       interval=AD(&t_dist, total_no_muts, no_muts_ij, 
      mut_to_node_map[i], mut_to_node_map[j],  ancestors[i], ancestors[j]); 
       fprintf(t_OUT, "%f\n", t_dist);
       /*
       fprintf(t_OUT, "%f\n", t_dist);
       */
