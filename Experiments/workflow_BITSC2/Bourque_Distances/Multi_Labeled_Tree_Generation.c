 ...
 * given in prufer format, which is a sequence of n-2 internal
 * nodes for a tree of n nodes.
 *
 *  Ccompile command: gcc Multi_Labeled_Tree_Generation.c  -o Generator
    Run command:   Generator <input_tree_file> <no_nodes>
 *
  *
 */
#include <stdio.h> 
...
void Decode(char *str, char a, short  *num, char *names[]){
   ...
   len1=0;
   for (i=0; i<len; i++) {
     if (str[i]!=a) { temp[len1]=str[i]; len1 +=1;}
     if (str[i]==a || i==len-1) {
            temp[len1]='\0';
...
short H_BD_Rooted_Jan(char p1[][MAXSIZE],  short mut_nos1,
  ...
  else if (common==0) return edge_nos1+edge_nos2+2; 
...
short  Compute_High_BD(short no_nodes1, short no_nodes2, char part1[][MAXSIZE],
        ...
        char part2[][MAXSIZE], short mut_nos2[], char *mut_pos2[][MAXSIZE], 
         short edge_nos2[], short edges2[][MAXSIZE]){
...
void Perturbation(edge *all_trees[], short graph[][MAXSIZE][2], short size, 
   short no_mut[], char *mutations[], short no_trees){
     ...
     for (i=0; i<no_trees; i++) {
        ...
        graph[i][0][0]=size;
        no_mut[i]=size+1;
        no_removed=rand()%(PROB); 
           /*printf("%d: no_removed\n", no_removed); */ 
        if (i!=0) {
        if (no_removed==10) {
            printf("%d: remove three nodes\n", i); 
          ...
          /* removed 2 */
           ...
           mutations[i][size]='0'; mutations[i][size-1]='0';
           no_mut[i]=size-2;
        } else if ( no_removed >3 && no_removed <6) {
           printf("%d: remove one nodes\n", i); 
           Remove(graph[i], size);
           mutations[i][size]='0';  
           no_mut[i]=size-1;
        } else { 
           /*printf("%d: remove no nodes\n", i); */ 
        }   
     }
  } /* i loop */
}
...
void Multiple_Labels(short graph[][MAXSIZE][2], char *label_names[][MAXSIZE],
 int no_trees){
     ...
     for (i=0; i<no_trees; i++) {
         /*printf("%d-th tree\n", i); */
        if (i==0) {Merge_Labels(graph[i], 0,  label_names[i]);}
        else {
        no_removed=rand()%PROB;
        if (no_removed==2) {
           /* printf("three_labels are merged\n"); */
           Merge_Labels(graph[i], 3, label_names[i]);
        } else if (no_removed==1 || no_removed==10 ||  no_removed==5) {
...
/*  compile command: gcc Multi_Labeled_Tree_Generation.c  -o Generator
 *  Run command:   Generator <input_tree_file> <no_nodes>
 */
...
short root;
  ...
  if (argc !=3 ) {
   printf("Command: ./a.out <tree_file> node_no\n"); exit(10);     }

   /* tree processing */
   In=fopen(argv[1], "r");
   if (In ==NULL) { printf("Tree_file_name is not readable\n"); exit(100);}
   no_nodes=atoi(argv[2]);
   ...
   while (fgets(line, 200, In)>0) {
     ...
     strcpy(h_line, line);
     while (strlen(h_line)>0) {
        sscanf(h_line, "%d", &node1);
        code[j]=node1; j=j+1;
        while (h_line[0]!=' ' && strlen(h_line)>1 )
            Remove_Space(h_line, strlen(h_line));
        Remove_Space(h_line, strlen(h_line));
      }
      all_trees[no_trees]=prufer_tree(code, no_nodes-2);

     /*
     if (no_trees==0){
       for (i=0; i<no1-1; i++) {
           printf("%d %d\n",  all_trees[no_trees][i].first, all_trees[no_trees][i].second);
       }
       printf("\n");
     }
       */
   ...
   for (i=0; i<no_trees; i++) {
     mutations[i]=(char *)malloc(no_nodes+1);
     for (k=0; k<no_nodes; k++) mutations[i][k]='1';
     mutations[i][no_nodes]='\0';
   }

   Perturbation(all_trees, graph, no_nodes-1,  no_mut, mutations,  no_trees);
   /* printf("                  here---------\n"); */
   Multiple_Labels(graph, label_names, no_trees);


   for (i=0; i<no_trees; i++) { 
      printf("#tree %d:  %d\n", i, graph[i][0][0]);
      /*
      Print_Trees(graph[i], label_names[i]); 
       */
 
      Distance_Comput_Jan(graph[i],  distance[i]);
      root=Orientation_Jan("00", label_names[i],  graph[i], distance[i]);
      Print_Trees(graph[i], label_names[i]); 
 }
