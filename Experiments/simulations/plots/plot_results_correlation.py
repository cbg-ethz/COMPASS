...
df = pd.read_csv("/home/e840r/Documents/COMPASS_project/out_simulations/correlation/results.csv")
d={"SNVs":[],"CNAs":[],"correlation":[],"seed":[],"method":[],"metric":[],"similarity":[]}
for i in df.index:
    for metric in ["MP3"]:
        if metric!="MP3-CNA" or int(df.loc[i,"nCNAs"])>0:
            ...
            d["correlation"].append(df.loc[i,"correlation"])
df = pd.DataFrame(d)
sns.set(font="Helvetica",font_scale=2.0)
g = sns.catplot(x="correlation", y="similarity",hue="method",\
    data=df, kind="box",hue_order=["COMPASS","BITSC2"],
        palette=["#E00105","#FDC508"],
    showmeans=True,meanprops={"markersize":10,"marker":"X"},
    margin_titles=False,legend=False,fliersize=2,height=4,aspect=1.5) # , height=4, aspect=.7
g.set_axis_labels("Correlation in region coverage","MP3")
...
plt.savefig("/home/e840r/Documents/COMPASS_project/Figures/simulations/correlation_res.svg",dpi=500,bbox_inches="tight",pad_inches=0.1)