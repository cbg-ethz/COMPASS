...
short CASet(float *t_dist, short no_muts, short no_muts_ij,  char *c_ances1[][MAXSIZE],
  ...
  printf("%d %d:\n     %s\n     %s\n", i, j,  c_ances1[i][j], c_ances2[i][j]); 
   ...
   if (no_muts_ij!=0 && no_muts_ij!=1) { 
       score=2*score; score=score/no_muts_ij; score=score/(no_muts_ij-1);
   }
   *t_dist=score;
   return 0;
}

 short Comput_mut_no(short no_muts, short map1[], short map2[]){
    short i;
    short ans;

     ans=0;
     for (i=0;i<no_muts; i++){ 
       if (map1[i]!=-1 || map2[i]!=-1) ans++;
     } 
     return ans;
 }

/*
 * Compile command: gcc -lm Pairwise_CASet_v2.c -o PW_CASet
 * run command:     ./PW_CASet <input_file> <output_file> <no_muts> 
...
short no_muts_ij;
  ...
  */
  ...
  Comput_Mut_Ancestors(root, graphs[i][0][0]+1, total_no_muts, distance[i], 
   ...
   for (i=0; i<count; i++) {
      for (j=i+1; j<count; j++) {     
        ...
        */
        t_dist=0.0;
       no_muts_ij=Comput_mut_no(total_no_muts, mut_to_node_map[i], mut_to_node_map[j]);
       interval=CASet(&t_dist, total_no_muts, no_muts_ij, common_ancestors[i],common_ancestors[j]); 
       fprintf(t_OUT, "%f\n", t_dist);
       /*
       fprintf(t_OUT, "%f\n", t_dist);
       */
