...
if len(args.i)>5 and args.i[-5:]==".loom": # Preprocess a single loom file
    convert_loom(args.i,args.o,15,6,0.2,0.7,0.4,0.015,region=args.region, reference=args.ref,verbose=True,SNP_file=args.SNP,panel_file = args.panel, whitelist_file = args.whitelist)
else: # Preprocess a whole directory of loom files
    for f in sorted(os.listdir(args.i)):
        if len(f)>5 and f[-5:]==".loom":
            convert_loom(os.path.join(args.i,f),args.o,15,6,0.2,0.7,0.4,0.015,region=args.region,reference=args.ref,verbose=True,SNP_file=args.SNP,panel_file = args.panel,whitelist_file=args.whitelist)
